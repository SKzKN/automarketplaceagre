...
from scrapers.resolver import resolve_one_listing
...
while True:
    # Always fetch from skip=0: as docs get updated they leave the query,
    # so the next fetch naturally gets the next batch of unresolved docs.
    listings = list(repo.car_listings.find(query).limit(batch_size))
    if not listings:
        break

    print(f'Processing batch ({processed} done so far, {len(listings)} in this batch)')
    ...
    # If nothing was updated in this batch, all remaining can't be resolved
    if batch_updated == 0:
        break
